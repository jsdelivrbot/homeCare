<template>
	<div class='view-findPassword'>
		...
		<group class='common-login-group register-group'>
			...
			<x-input name="password" type='password' placeholder="新密码（6-14位数字/字母/下划线）" v-model='password' v-validate="'required|min:6|max:14|noSpecial'" :class="{'is-danger': validateError.has('password')}">
				<img slot="label" src="~assets/images/dl_mm.png">
			</x-input>
			<x-input name="password2" type='password' placeholder="重复新密码" v-model='password2' v-validate="'required|min:6|max:14|noSpecial'" :class="{'is-danger': validateError.has('password2')}">
				<img slot="label" src="~assets/images/dl_mm.png">
			</x-input>
		</group>
		<x-button class='common-login-btn register-btn' @click.native="submitData" :type="validateError.any()?'default':'primary'" :disabled='validateError.any()'> 重置密码</x-button>
	</div>
</template>
