<template>
	<div class='view-register'>
		<x-header :title='$route.name' :left-options="{backText: ''}"></x-header>
		<div class="title">
			请输入注册信息
		</div>
		<group class='common-login-group register-group'>
			...
			<x-input name="password" type='password' placeholder="密码" v-model='password' v-validate="'required'" :class="{'is-danger': validateError.has('password')}">
				<img slot="label" src="~assets/images/dl_mm.png">
			</x-input>
			<x-input name="password2" type='password' placeholder="重复密码" v-model='password2' v-validate="'required'" :class="{'is-danger': validateError.has('password2')}">
				<img slot="label" src="~assets/images/dl_mm.png">
			</x-input>
		</group>
		<x-button class='common-login-btn register-btn' @click.native="submitData" :type="validateError.any()?'default':'primary'" :disabled='validateError.any()'> 注册并登录</x-button>
		<p class='register-clause'>
			注册视同您已经阅读并完全同意
			<router-link :to="{path:'/404'}">《使用条款》</router-link>
		</p>
		<!--<div class="common-illustration"></div>-->
	</div>
</template>
...
<script>
	...
	export default {
		...
		mounted() {
			this.$validator.validateAll()
//			document.querySelector('.putRecourse').style.display = 'none';
		},
		components: {
			XInput,
			Group,
