<template>
	<div class='view-contract-doctor'>
		<!--<div class="common-header-right">
			<router-link v-show='isSign=="0"' :to="{path:'/homeRouterView/hospital'}">添加</router-link>
		</div>-->
				...
				<ul class="clearfix">
					<li v-for='(t,i) in filterList'>
						<router-link :to="{path: '/homeRouterView/contractDoctorDetails',query:{id:t.ID,isSign:isSign,userId:$route.query.userId}}">
							...
							<rater v-model="t.ServiceStar" slot="value" active-color="#FF9900" :margin="4"></rater>
							</h3>
								<p>
...
<script>
	...
	export default {
		...
		computed: {
			...
			//是否签约 0未签约1已签约
				isSign(){
					return this.$route.query.departmentId||!this.$route.query.userId?0:1
				},
			ajaxData() {
				return {
					header: {
					...
					body: {
						userid: this.$route.query.userId||this.$common.getLocalStorage('user').doctorid,
						SignType: this.isSign ?'1':'0',
