<template>
  <div class='view-register'>
    ...
    <x-button class='common-login-btn register-btn'
      ...
      :disabled='validateError.any()'> 注册并登录</x-button>
    <p class='register-clause'>
      注册视同您已经阅读并完全同意
      <router-link :to="{path:'/termsOfUse'}">《使用条款》</router-link>
    </p>
    ...
    login() {
      ...
      this.$ajax({
        data: {
          ...
          body: {
            mobile: this.tel,
            pwd: sha512(this.password),
            openId:localStorage.openId
          }
        },
        success: res => {
          this.$common.setLocalStorage("token", res.tokenid);
          this.$common.setLocalStorage("userPic", res.HeaderPhoto);
          this.$common.setLocalStorage("user", res);
          this.$router.push({
