	...
	<div class='view-visit-details'>
		...
		<div class="comment">
			<group class="common-menu-list">
				<cell title="星级：">
					<rater v-model="submitData.Star"
					    active-color="#FF9900"
					    :margin="4"
					    slot='child'
					    :disabled='isComment'></rater>
				</cell>
				<x-textarea v-model="submitData.Content"
				    placeholder='评论内容'
				    :readonly='isComment'></x-textarea>
			</group>
			<!--<x-button v-if='isComment'  class='common-login-btn register-btn' text='删除评论' @click.native="deleteComment" type="warn" ></x-button>-->
			<x-button v-if='!isComment'
			    class='common-login-btn register-btn'
			    text='提交评论'
  ...
  computed: {
    ...
    submitData() {
      return {
        ...
        Star: this.isComment
          ? parseFloat(this.$route.query.details.CommentStar)
          : 0
      };
    }
  },
