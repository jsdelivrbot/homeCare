<template>
	<div class='view-contract-doctor-evaluate'>
		<group class="common-menu-list">
			<cell title="评价总数：" :value='doctorEvaluation.TotalCount+"条"'></cell>
			<cell title="平均星级：">
				<rater v-model="doctorEvaluation.AveStar" active-color="#FF9900" :margin="4" slot='child' disabled></rater>
			</cell>
		</group>
		<scroller ref='scroller' class='common-scroller' :on-refresh="refresh" :on-infinite="infinite">
			...
			<div class="list">
				<group class="common-menu-list">
					<cell v-for='t in list' title="评价星级：" :inline-desc='t.Content' :value='t.AddTime.split(" ")[0]' :key='t.ID'>
						<rater v-model="t.ServiceStar" active-color="#FF9900" :margin="4" slot='icon' disabled></rater>
					</cell>
				</group>
...
<script>
	import { XHeader, Rater, Spinner, Group, Cell } from 'vux'
