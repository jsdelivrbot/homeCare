<template>
	<div class='view-doctor-details common-padding'>
		<div class="common-header-right">
			<a v-if='!$route.query.userId'
			    @click='confirm=true'>{{isSign?'解除签约':'签约'}}</a>
		</div>
		<div class="common-user-details">
			<rater v-model="details.ServiceStar"
			    active-color="#FF9900"
			    :margin="4"
			    disabled></rater>
		...
		<group class="common-menu-list">
			...
			<cell title="执业点"
				...
				<img src="~assets/images/contract-doctor-zyd.png"
				    slot='icon' />
			</cell>
			<cell title="签约患者数"
			    :value='details.SignUserCount'>
				<img src="~assets/images/contract-doctor-qyhz.png"
				    slot='icon' />
			</cell>
			<cell :title="'患者评价( '+doctorEvaluation+' )'"
			    is-link
			    :link="{path:'/homeRouterView/contractDoctorEveluate',query:{id:details.ID}}">
				<img src="~assets/images/contract-doctor-hzpj.png"
				    slot='icon' />
			</cell>
		</group>
