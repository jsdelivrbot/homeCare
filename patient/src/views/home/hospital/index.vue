<template>
	<div class='view-hospital'>
		<div class="common-header-right">
			<spinner type="ios" v-if='addressLoading'></spinner>
			<div class="address-select" v-else>
				<a>{{location.address.district}}</a>
				<x-address class='addressSelect' @on-hide='replaceText' ref='addressSelect' title='城市' v-model="addressSelect" :list="addressData" placeholder="请选择城市"></x-address>
			</div>
		</div>
		<div class="common-search">
			<input type="search" v-model='search' placeholder="请输入医院名称" @keyup.enter="getData({search:true})" />
		</div>
		<div class="list">
			...
			<ul class="clearfix" v-else>
				<li v-for='t in list'>
					<router-link :to="{path: '/homeRouterView/hospitalDepartment',query:{id:t.ID}}">
						...
						<div class="text">
							<h1>{{t.AgencyName}} <span>{{t.AgencyLevel}}</span></h1>
							<p>医院性质：<span>{{t.AgencyProp}}</span></p>
							<p class="address">医院地址：<span>{{t.Address}}</span></p>
						</div>
					</router-link>
				</li>
				<div class="common-no-data" v-if='list.length<=0'>
					没有更多数据
				</div>
			</ul>
...
<script>
	import { XHeader, Spinner, XAddress } from 'vux'
	export default {
		name: 'hospital',
		data() {
			return {
		...
		methods: {
			//替换右上角地址文字
			replaceText() {
				this.location.address.district = this.$refs.addressSelect.nameValue.split(' ')[2];
				this.getData();
			},
			//获取医院列表
			getData(opt) {
